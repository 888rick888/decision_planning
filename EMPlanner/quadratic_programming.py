...
class qp:
    ...
    def add_density_qp(self, qp_path_s,qp_path_l,qp_path_dl,qp_path_ddl):
        # 该函数将增密qp_path
        n_init = 20 #增密的点的个数
        
        n = 501

        qp_path_s_final = np.zeros(n,1)
        qp_path_l_final = np.zeros(n,1)
        qp_path_dl_final = np.zeros(n,1)
        qp_path_ddl_final = np.zeros(n,1)
        ds = (qp_path_s(-1) - qp_path_s(1))/(n-1)
        index = 1
        for i in range(n):
            x = qp_path_s[0] + i * ds
            qp_path_s_final[i] = x
            while x >= qp_path_s[index]:    #while 循环退出的条件是x<qp_path_s(index)，所以x对应的前一个s的编号是index-1 后一个编号是index
                index = index + 1
                if index == n_init:
                    break
            pre = index - 1
            ...
            qp_path_ddl_final[i] = ddl_pre + (ddl_cur - ddl_pre) * delta_s/(qp_path_s[cur] - qp_path_s[cur])
